...
class graphLedger(val module: DefModule) {
  ...
  */
  ...
  def getInstances: Set[WDefInstance] = {
    val nonInst = defInstances.find {
      case _: WDefInstance => false
      case _ => true
    }
    nonInst.foreach(inst => throw new Exception(s"${inst.serialize} is not WDefInstance class\n"))
    defInstances.collect { case inst: WDefInstance => inst }.toSet
  }
