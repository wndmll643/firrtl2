...
object HierCovUtil {
  ...
  def selectInputBits(ports: Seq[Port], params: HierCovParams): Seq[(Expression, String)] = {
  ...
  def selectRegBits(regs: Seq[DefRegister], params: HierCovParams): Seq[(Expression, String)] = {
...
class HierModuleInfo(val mName: String,
...
object HierModuleInfo {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfo = {
    ...
    new HierModuleInfo(mod.name, insts, regs)
  }
}

class InstrHierCov(mod: DefModule,
...
class InstrHierAssert(mod: DefModule, mInfo: HierModuleInfo) {
  private val mName = mod.name
...
class InstrHierReset(mod: DefModule, mInfo: HierModuleInfo, moduleInfos: scala.collection.Map[String, HierModuleInfo]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage extends Transform {
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCov(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssert(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierReset(m, moduleInfos(m.name), moduleInfos)
