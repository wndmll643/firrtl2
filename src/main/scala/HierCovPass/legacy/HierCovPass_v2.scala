...
object HierCovUtilV2 {
  ...
  def typeWidth(tpe: Type): Int = tpe match {
    case UIntType(IntWidth(w)) => w.toInt
    case SIntType(IntWidth(w)) => w.toInt
    case _ => throw new Exception(s"Unsupported type width: ${tpe.serialize}")
  }

  def bitExtract(expr: Expression, idx: Int, tpe: Type): Expression = {
    val width = typeWidth(tpe)
  ...
  def selectInputBits(ports: Seq[Port], params: HierCovParamsV2): Seq[(Expression, String)] = {
  ...
  def selectRegBits(regs: Seq[DefRegister], params: HierCovParamsV2): Seq[(Expression, String)] = {
    val bits = ListBuffer[(Expression, String)]()
    for (reg <- regs) {
      val width = typeWidth(reg.tpe)
...
class HierModuleInfoV2(val mName: String,
                       val insts: scala.collection.Set[WDefInstance],
                       val regs: scala.collection.Set[DefRegister])

object HierModuleInfoV2 {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV2 = {
    val insts = gLedger.getInstances
    val regs = gLedger.findRegs
    new HierModuleInfoV2(mod.name, insts, regs)
  }
}

class InstrHierCovV2(mod: DefModule,
                     mInfo: HierModuleInfoV2,
                     extModules: Set[String],
                     params: HierCovParamsV2) {
  import HierCovUtilV2._
...
class InstrHierAssertV2(mod: DefModule, mInfo: HierModuleInfoV2) {
  private val mName = mod.name
...
class InstrHierResetV2(mod: DefModule, mInfo: HierModuleInfoV2, moduleInfos: scala.collection.Map[String, HierModuleInfoV2]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v2 extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV2]()
  private val params = HierCovParamsV2()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV2(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV2(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV2(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV2(m, moduleInfos(m.name), moduleInfos)
