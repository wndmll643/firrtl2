// hier_cov.hierCoverage_v11a — v9a + tree-sum aggregation port.
//
// Selection : control inputs + control regs (mux-source-derived)  — same as v9a
// Hashing   : direct-or-fold                                       — same as v9a
// Sizing    : min(numBits, cap), capped at maxAddrWidth=20         — same as v9a
...
//
// Per-module bitmaps and the existing `io_hierCovHash` chain are kept
// untouched: each module still sees its direct children in its own
// address (entanglement at one hierarchy hop is intentional). The new
// port is purely additive — the bit-identity of v9a's emitted Verilog
// is preserved (v9a continues to default `emitSumTotal = false`).
package hier_cov
...
class hierCoverage_v11a extends Transform {
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    val text =
      ...
      s"Total coverage points (hier_cov_v11a ctrl-input, tree-sum): ${totalCov}\n" +
