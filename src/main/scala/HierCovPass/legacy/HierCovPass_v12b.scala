// hier_cov.hierCoverage_v12b — v11b + own-state hash addressing chain.
//
// Selection : control input ports (mux-source) + control regs   — same as v6b/v11b
// Hashing   : bucket-XOR-reduce                                  — same as v6b/v11b
// Sizing    : min(numBits, cap), maxAddrWidth=12                 — same as v6b/v11b
// Hash pipe : 6-bit io_hierCovHashOwn to parents (NEW: own-state-only).
//             Bucket-histogram-driven `io_hierCovHash` still emitted but
//             ignored by parents — pruned by FIRRTL DCE.
// Aggreg.   : `io_hierCovSumTotal` tree-sum port (inherited from v11b).
//
// v12b is the bucketHash counterpart to v12a, paired so the own-hash
// chain can be ablated independently against each baseline hashing
// strategy. See HierCovPass_v12a.scala for the design rationale.
package hier_cov
...
class hierCoverage_v12b extends Transform {
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    val text =
      ...
      s"Total coverage points (hier_cov_v12b ctrl-input, tree-sum, own-hash chain): ${totalCov}\n" +
