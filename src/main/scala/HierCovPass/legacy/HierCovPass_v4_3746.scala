...
case class HierCovParamsV4_3746(
  inputHashSize: Int = 6,
  coreHashSize: Int = 8,
...
object HierCovUtilV4_3746 {
  ...
  def selectInputBits(ports: Seq[Port], params: HierCovParamsV4_3746): Seq[(Expression, String)] = {
  ...
  def selectRegBits(regs: Seq[DefRegister], params: HierCovParamsV4_3746): Seq[(Expression, String)] = {
...
class HierModuleInfoV4_3746(val mName: String,
                            val insts: scala.collection.Set[WDefInstance],
                            val regs: scala.collection.Set[DefRegister])

object HierModuleInfoV4_3746 {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV4_3746 = {
    val insts = gLedger.getInstances
    val regs = gLedger.findRegs
    new HierModuleInfoV4_3746(mod.name, insts, regs)
  }
}

class InstrHierCovV4_3746(mod: DefModule,
                          mInfo: HierModuleInfoV4_3746,
                          extModules: Set[String],
                          params: HierCovParamsV4_3746) {
  import HierCovUtilV4_3746._
...
class InstrHierAssertV4_3746(mod: DefModule, mInfo: HierModuleInfoV4_3746) {
  private val mName = mod.name
...
class InstrHierResetV4_3746(mod: DefModule, mInfo: HierModuleInfoV4_3746, moduleInfos: scala.collection.Map[String, HierModuleInfoV4_3746]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v4_3746 extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV4_3746]()
  private val params = HierCovParamsV4_3746()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV4_3746(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV4_3746(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV4_3746(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV4_3746(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV4_3746.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV4_3746.selectInputBits(m.ports, params)
            val regBits = HierCovUtilV4_3746.selectRegBits(mInfo.regs.toSeq, params)
