// v8a: transition coverage (AFL-style edge tracking) + cone+origin register filtering.
// Based on v7 (data-input approach) with two improvements:
//
// Changes vs v7:
//   - Cone+origin register filtering: instead of using all mux-feeding registers (ctrlRegs),
//     uses only registers in the backward cone of narrow output ports AND not input-derived.
//     (graphLedger.findControlConeRegs - findInputDerivedRegs = v8Regs)
//   - AFL-style transition coverage: tracks state TRANSITIONS, not just unique states.
//     Each module stores prev_addr register. Coverage map indexed by
//     transition_addr = addr XOR (prev_addr >> 1), making (A->B) and (B->A) distinct.
//   - Keeps v7's data-input selection, dynamic log2 hash sizing, extmodule proxy, raised caps.
package hier_cov
...
case class HierCovParamsV8a(
  ...
  maxControlOutputWidth: Int = 16, // cone filter: narrow output port threshold
  maxOriginHops: Int = 3           // origin filter: N-hop depth for input-derived classification
)

object HierCovUtilV8a {
  ...
  def selectDataInputBits(ports: Seq[Port], ctrlPortNames: Set[String], params: HierCovParamsV8a): Seq[(Expression, String)] = {
...
class HierModuleInfoV8a(
  ...
  val coneRegs: scala.collection.Set[DefRegister],        // regs in backward cone of narrow outputs
  val inputDerivedRegs: scala.collection.Set[DefRegister], // input-derived regs (N-hop)
  val v8Regs: scala.collection.Set[DefRegister]            // coneRegs - inputDerivedRegs
)

object HierModuleInfoV8a {
  def apply(mod: DefModule, gLedger: graphLedger, params: HierCovParamsV8a): HierModuleInfoV8a = {
    ...
    val coneRegs = gLedger.findControlConeRegs(params.maxControlOutputWidth)
    val inputDerivedRegs = gLedger.findInputDerivedRegs(coneRegs, params.maxOriginHops)
    val v8Regs = coneRegs -- inputDerivedRegs
    new HierModuleInfoV8a(mod.name, insts, regs, ctrlRegs, dirInRegs, ctrlPortNames, coneRegs, inputDerivedRegs, v8Regs)
  }
}

class InstrHierCovV8a(mod: DefModule,
                      mInfo: HierModuleInfoV8a,
                      extModules: Set[String],
                      extModulePorts: scala.collection.Map[String, Seq[Port]],
                      params: HierCovParamsV8a) {
  import HierCovUtilV8a._
  ...
  def instrument(): DefModule = {
    mod match {
      case m: Module =>
        ...
        val regBits = selectControlRegBits(mInfo.v8Regs, mInfo.dirInRegs, params)
        ...
        if (hasClockFound && addrWidth > 0) {
          ...
          // AFL-style transition tracking: prev_addr stores last cycle's addr
          val prevAddr = DefRegister(NoInfo, s"${mName}_prevAddr", UIntType(IntWidth(addrWidth)),
            WRef(clockName, ClockType, PortKind, SourceFlow),
            UIntLiteral(0, IntWidth(1)),
            WRef(s"${mName}_prevAddr", UIntType(IntWidth(addrWidth)), RegKind, UnknownFlow))

          // transition_addr = addr XOR (prev_addr >> 1)
          val shiftedPrevWidth = Math.max(1, addrWidth - 1)
          val shiftedPrevWire = DefWire(NoInfo, s"${mName}_shiftedPrev", UIntType(IntWidth(addrWidth)))
          val shiftedPrevRaw = DoPrim(Shr, Seq(WRef(prevAddr)), Seq(1), UIntType(IntWidth(shiftedPrevWidth)))
          val shiftedPrevPadded = DoPrim(Pad, Seq(shiftedPrevRaw), Seq(addrWidth), UIntType(IntWidth(addrWidth)))
          val shiftedPrevCon = Connect(NoInfo, WRef(shiftedPrevWire), shiftedPrevPadded)

          val transAddrWire = DefWire(NoInfo, s"${mName}_transAddr", UIntType(IntWidth(addrWidth)))
          val transAddrExpr = DoPrim(Xor, Seq(addrExpr, WRef(shiftedPrevWire)), Seq(), UIntType(IntWidth(addrWidth)))
          val transAddrCon = Connect(NoInfo, WRef(transAddrWire), transAddrExpr)

          val prevAddrUpdate = Connect(NoInfo, WRef(prevAddr), addrExpr)
          ...
          val rdAddr = Connect(NoInfo, WSubField(readSubField, "addr", UIntType(IntWidth(addrWidth)), SinkFlow), WRef(transAddrWire))
          ...
          val wrAddr = Connect(NoInfo, WSubField(writeSubField, "addr", UIntType(IntWidth(addrWidth)), SinkFlow), WRef(transAddrWire))
          ...
          val newStmts = stmts ++ inputHashStmts ++ coreHashStmts ++ Seq(covMap, covSum, prevAddr) ++
            Seq(shiftedPrevWire, shiftedPrevCon, transAddrWire, transAddrCon, prevAddrUpdate) ++
...
class InstrHierAssertV8a(mod: DefModule, mInfo: HierModuleInfoV8a) {
  private val mName = mod.name
...
class InstrHierResetV8a(mod: DefModule, mInfo: HierModuleInfoV8a, moduleInfos: scala.collection.Map[String, HierModuleInfoV8a]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v8a extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV8a]()
  private val params = HierCovParamsV8a()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV8a(m, gLedger, params)
    }
    ...
    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV8a(m, moduleInfos(m.name), extModules, extModulePorts, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV8a(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV8a(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], extModulePorts: scala.collection.Map[String, Seq[Port]], topName: String): Unit = {
    ...
    def dynamicHash(n: Int, maxH: Int): Int = {
      ...
      else Math.max(params.minHashSize, Math.min(maxH, HierCovUtilV8a.log2Ceil(n + 1)))
    }

    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV8a.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV8a.selectDataInputBits(m.ports, mInfo.ctrlPortNames, params)
            val regBits = HierCovUtilV8a.selectControlRegBits(mInfo.v8Regs, mInfo.dirInRegs, params)
    ...
    val perModule = moduleInfos.keys.toSeq.sorted.map { mName =>
      ...
      s"  ${mName}: covMapSize=${covSize}, inputHash=${inputHashH}, coreHash=${coreHashH}, ctrlRegs=${ctrlRegCount}, coneRegs=${mInfo.coneRegs.size}, inputDerivedRegs=${mInfo.inputDerivedRegs.size}, v8Regs=${mInfo.v8Regs.size}, totalRegs=${totalRegCount}, extProxyBits=${extBitCount}, instances=${instCnt}\n"
    }
    ...
    val text =
      ...
      s"Total coverage points (hier_cov_v8a transition+cone data-input): ${totalCov}\n" +
