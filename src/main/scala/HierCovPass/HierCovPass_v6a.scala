// v6a: redesigned hierCoverage — control registers only (core hash) + data input ports
// (non-control inputs) for input hash. Eliminates noise from arithmetic/operand registers
// by using only registers that feed mux select signals (via graphLedger.findMuxSrcs).
//
// Changes vs v4_3746_fix:
//   - HierModuleInfoV6a: adds ctrlRegs, dirInRegs, ctrlPortNames (from findMuxSrcs/findDirInRegs)
//   - selectControlRegBits: uses ctrlRegs only, excludes dirInRegs and wide regs (>= 20 bits)
//   - selectDataInputBits: excludes ports in ctrlSrcs("Port") (pure data/operand inputs only)
//   - HierCovParamsV6a: coreHashSize reduced from 8 to 6 (fewer bits in → smaller map, better density)
//   - writeCoverageSummary: reports ctrlRegs and totalRegs per module
//   - SInt fix (same as v4_3746_fix): SIntLiteral(0) for SInt regs in metaReset
//   - writeCoverageSummary (same as v4_3746): writes {topName}_hier_cov_summary.txt
package hier_cov
...
case class HierCovParamsV6a(
  maxInputHashSize: Int = 6,  // cap for dynamic input hash (actual size = min(cap, numBits))
...
object HierCovUtilV6a {
  ...
  def selectDataInputBits(ports: Seq[Port], ctrlPortNames: Set[String], params: HierCovParamsV6a): Seq[(Expression, String)] = {
...
object HierModuleInfoV6a {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV6a = {
    ...
    new HierModuleInfoV6a(mod.name, insts, regs, ctrlRegs, dirInRegs, ctrlPortNames)
  }
}

class InstrHierCovV6a(mod: DefModule,
                      mInfo: HierModuleInfoV6a,
                      extModules: Set[String],
                      params: HierCovParamsV6a) {
  import HierCovUtilV6a._
...
class InstrHierAssertV6a(mod: DefModule, mInfo: HierModuleInfoV6a) {
  private val mName = mod.name
...
class InstrHierResetV6a(mod: DefModule, mInfo: HierModuleInfoV6a, moduleInfos: scala.collection.Map[String, HierModuleInfoV6a]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v6a extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV6a]()
  private val params = HierCovParamsV6a()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV6a(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV6a(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV6a(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV6a(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV6a.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV6a.selectDataInputBits(m.ports, mInfo.ctrlPortNames, params)
            val regBits = HierCovUtilV6a.selectControlRegBits(mInfo.ctrlRegs, mInfo.dirInRegs, params)
