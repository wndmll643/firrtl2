// hier_cov.hierCoverage_ctrl_bucket_tree_sorted — Gate-3-validation variant.
//
// Identical to ctrl_bucket_tree EXCEPT it uses the sorted selector variants
...
// the CIRCT port's --hier-cov-ctrl-bucket-tree-sorted variant.
//
// IMMUTABILITY: existing ctrl_bucket_tree stays untouched; this is a NEW
...
class hierCoverage_ctrl_bucket_tree_sorted extends Transform {
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val instrCircuit = circuit map { m: DefModule =>
      ...
      val inputBits = HierCovSelectors.selectControlInputBitsSorted(ports, mInfo.ctrlPortNames, params)
    ...
    BucketManifest.maybeEmitWithSignals(
      metaResetCircuit, metaResetCircuit.main, moduleInfos, "ctrl_bucket_tree_sorted",
      (mName: String) => moduleMapForSignals.get(mName) match {
        case Some(m: Module) =>
          val mInfo     = moduleInfos(mName)
          val inputBits = HierCovSelectors.selectControlInputBitsSorted(m.ports, mInfo.ctrlPortNames, params)
          val regBits   = HierCovSelectors.selectControlRegBitsSorted(mInfo.ctrlRegs, mInfo.dirInRegs, params)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    val text =
      ...
      s"Total coverage points (hier_cov ctrl_bucket_tree_sorted, ctrl-input, tree-sum): ${totalCov}\n" +
