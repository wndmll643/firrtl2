// v9a: eliminate hashing variant based on v6b.
...
// Widens maxCoreHashSize from 6 to 14, adds maxAddrWidth=20 cap.
// Widens submodHashSize from 6 to 16 for better hierarchy information flow.
package hier_cov
...
case class HierCovParamsV9a(
  maxInputHashSize: Int = 6,     // v6b-style fixed
  maxCoreHashSize: Int = 14,     // widened from 6 to allow wider address
  maxAddrWidth: Int = 20,        // NEW: hard cap on total address width (2^20 = 1M entries)
  submodHashSize: Int = 16,      // widened from 6 to reduce hierarchy bottleneck
  covSumSize: Int = 32,
  maxInputPorts: Int = 8,
  maxBitsPerPort: Int = 8,
  maxRegBits: Int = 64,          // v6b-style
...
object HierCovUtilV9a {
  ...
  def selectControlInputBits(ports: Seq[Port], ctrlPortNames: Set[String], params: HierCovParamsV9a): Seq[(Expression, String)] = {
  ...
  // v9a: direct concatenation when bits fit, XOR-fold fallback when they don't.
  // Sorts bits by stableHash(name) for deterministic ordering.
  // When bits.size <= outWidth: zero-padded concatenation — ZERO information loss.
  // When bits.size > outWidth: XOR-fold to outWidth.
...
object HierModuleInfoV9a {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV9a = {
    ...
    new HierModuleInfoV9a(mod.name, insts, regs, ctrlRegs, dirInRegs, ctrlPortNames)
  }
}

class InstrHierCovV9a(mod: DefModule,
                      mInfo: HierModuleInfoV9a,
                      extModules: Set[String],
                      params: HierCovParamsV9a) {
  import HierCovUtilV9a._
  ...
  def instrument(): DefModule = {
    mod match {
      case m: Module =>
        ...
        // v9a: use min(numBits, maxSize) — NO log2 compression
...
class InstrHierAssertV9a(mod: DefModule, mInfo: HierModuleInfoV9a) {
  private val mName = mod.name
...
class InstrHierResetV9a(mod: DefModule, mInfo: HierModuleInfoV9a, moduleInfos: scala.collection.Map[String, HierModuleInfoV9a]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v9a extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV9a]()
  private val params = HierCovParamsV9a()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV9a(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV9a(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV9a(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV9a(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV9a.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV9a.selectControlInputBits(m.ports, mInfo.ctrlPortNames, params)
            val regBits = HierCovUtilV9a.selectControlRegBits(mInfo.ctrlRegs, mInfo.dirInRegs, params)
            ...
            if (dynamicInput + dynamicCore > params.maxAddrWidth) {
              ...
              if (dynamicCore < 0) {
                dynamicInput = params.maxAddrWidth
                dynamicCore = 0
              }
            }
