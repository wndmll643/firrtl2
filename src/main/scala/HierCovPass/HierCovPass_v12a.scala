// hier_cov.hierCoverage_v12a — v11a + own-state hash addressing chain.
//
// Selection : control inputs + control regs (mux-source-derived)  — same as v9a/v11a
// Hashing   : direct-or-fold                                       — same as v9a/v11a
// Sizing    : min(numBits, cap), capped at maxAddrWidth=20         — same as v9a/v11a
// Hash pipe : 16-bit io_hierCovHashOwn to parents (NEW: own-state-only,
//             not bucket-histogram). Bucket-histogram-driven `io_hierCovHash`
//             still emitted but ignored by parents — pruned by FIRRTL DCE.
// Aggreg.   : `io_hierCovSumTotal` tree-sum port (inherited from v11a).
//
// Why: v11a's parent-addressing chain uses children's `io_hierCovHash`,
// which is built from each child's bucket histogram and therefore encodes
// transitive descendant *bitmap-exploration history* into the grandparent's
// address. v12a's own-state hash depends only on the child's own input +
// reg bits, so a grandparent's bitmap counts genuine
// (parent_state, direct_child_state) pairs without grandchildren leaking
// through. Deep-exploration coverage is preserved cleanly via the v11
// `io_hierCovSumTotal` aggregation — no information is lost overall.
package hier_cov
...
class hierCoverage_v12a extends Transform {
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val instrCircuit = circuit map { m: DefModule =>
      ...
      new InstrHierCov(
        ...
        inputBits, regBits, HierCovHash.directOrFold,
        emitSumTotal    = true,
        useOwnHashChain = true
      ).instrument()
    }
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    val text =
      ...
      s"Total coverage points (hier_cov_v12a ctrl-input, tree-sum, own-hash chain): ${totalCov}\n" +
