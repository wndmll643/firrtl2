// v9d: XOR-fold hash variant based on v7b.
// Replaces XOR-reduce bucket hash with XOR-fold. Widens submodHashSize from 6 to 16.
// Keeps v7b features: dynamic log2 hash sizing, extmodule I/O proxy, raised caps.
package hier_cov
...
case class HierCovParamsV9d(
  ...
  submodHashSize: Int = 16,    // widened from 6 — XOR-fold needs wider signature
  covSumSize: Int = 32,
  maxInputPorts: Int = 8,
...
object HierCovUtilV9d {
  ...
  def selectControlInputBits(ports: Seq[Port], ctrlPortNames: Set[String], params: HierCovParamsV9d): Seq[(Expression, String)] = {
...
object HierModuleInfoV9d {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV9d = {
    ...
    new HierModuleInfoV9d(mod.name, insts, regs, ctrlRegs, dirInRegs, ctrlPortNames)
  }
}

class InstrHierCovV9d(mod: DefModule,
                      mInfo: HierModuleInfoV9d,
                      extModules: Set[String],
                      extModulePorts: scala.collection.Map[String, Seq[Port]],
                      params: HierCovParamsV9d) {
  import HierCovUtilV9d._
...
class InstrHierAssertV9d(mod: DefModule, mInfo: HierModuleInfoV9d) {
  private val mName = mod.name
...
class InstrHierResetV9d(mod: DefModule, mInfo: HierModuleInfoV9d, moduleInfos: scala.collection.Map[String, HierModuleInfoV9d]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v9d extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV9d]()
  private val params = HierCovParamsV9d()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV9d(m, gLedger)
    }
    ...
    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV9d(m, moduleInfos(m.name), extModules, extModulePorts, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV9d(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV9d(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], extModulePorts: scala.collection.Map[String, Seq[Port]], topName: String): Unit = {
    ...
    def dynamicHash(n: Int, maxH: Int): Int = {
      ...
      else Math.max(params.minHashSize, Math.min(maxH, HierCovUtilV9d.log2Ceil(n + 1)))
    }

    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV9d.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV9d.selectControlInputBits(m.ports, mInfo.ctrlPortNames, params)
            val regBits = HierCovUtilV9d.selectControlRegBits(mInfo.ctrlRegs, mInfo.dirInRegs, params)
    ...
    val text =
      ...
      s"Total coverage points (hier_cov_v9d control-input): ${totalCov}\n" +
