// v8b: transition coverage + cone+origin filtering + control-input (v6b-style).
// Based on v8a with one change: input hash uses ONLY control input ports
// (ports that feed mux selects) instead of data input ports.
// All other v8a features preserved: cone+origin register filtering, AFL-style
// transition coverage, dynamic log2 hash sizing, extmodule proxy, raised caps.
package hier_cov
...
object HierCovUtilV8b {
  ...
  def selectControlInputBits(ports: Seq[Port], ctrlPortNames: Set[String], params: HierCovParamsV8b): Seq[(Expression, String)] = {
...
object HierModuleInfoV8b {
  def apply(mod: DefModule, gLedger: graphLedger, params: HierCovParamsV8b): HierModuleInfoV8b = {
    ...
    new HierModuleInfoV8b(mod.name, insts, regs, ctrlRegs, dirInRegs, ctrlPortNames, coneRegs, inputDerivedRegs, v8Regs)
  }
}

class InstrHierCovV8b(mod: DefModule,
                      mInfo: HierModuleInfoV8b,
                      extModules: Set[String],
                      extModulePorts: scala.collection.Map[String, Seq[Port]],
                      params: HierCovParamsV8b) {
  import HierCovUtilV8b._
...
class InstrHierAssertV8b(mod: DefModule, mInfo: HierModuleInfoV8b) {
  private val mName = mod.name
...
class InstrHierResetV8b(mod: DefModule, mInfo: HierModuleInfoV8b, moduleInfos: scala.collection.Map[String, HierModuleInfoV8b]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v8b extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV8b]()
  private val params = HierCovParamsV8b()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV8b(m, gLedger, params)
    }
    ...
    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV8b(m, moduleInfos(m.name), extModules, extModulePorts, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV8b(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV8b(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], extModulePorts: scala.collection.Map[String, Seq[Port]], topName: String): Unit = {
    ...
    def dynamicHash(n: Int, maxH: Int): Int = {
      ...
      else Math.max(params.minHashSize, Math.min(maxH, HierCovUtilV8b.log2Ceil(n + 1)))
    }

    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV8b.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV8b.selectControlInputBits(m.ports, mInfo.ctrlPortNames, params)
            val regBits = HierCovUtilV8b.selectControlRegBits(mInfo.v8Regs, mInfo.dirInRegs, params)
    ...
    val text =
      ...
      s"Total coverage points (hier_cov_v8b transition+cone control-input): ${totalCov}\n" +
