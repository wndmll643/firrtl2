...
object HierCovUtilV4 {
  ...
  def selectInputBits(ports: Seq[Port], params: HierCovParamsV4): Seq[(Expression, String)] = {
  ...
  def selectRegBits(regs: Seq[DefRegister], params: HierCovParamsV4): Seq[(Expression, String)] = {
...
class HierModuleInfoV4(val mName: String,
                       val insts: scala.collection.Set[WDefInstance],
                       val regs: scala.collection.Set[DefRegister])

object HierModuleInfoV4 {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV4 = {
    val insts = gLedger.getInstances
    val regs = gLedger.findRegs
    new HierModuleInfoV4(mod.name, insts, regs)
  }
}

class InstrHierCovV4(mod: DefModule,
                     mInfo: HierModuleInfoV4,
                     extModules: Set[String],
                     params: HierCovParamsV4) {
  import HierCovUtilV4._
...
class InstrHierAssertV4(mod: DefModule, mInfo: HierModuleInfoV4) {
  private val mName = mod.name
...
class InstrHierResetV4(mod: DefModule, mInfo: HierModuleInfoV4, moduleInfos: scala.collection.Map[String, HierModuleInfoV4]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v4 extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV4]()
  private val params = HierCovParamsV4()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV4(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV4(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV4(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV4(m, moduleInfos(m.name), moduleInfos)
