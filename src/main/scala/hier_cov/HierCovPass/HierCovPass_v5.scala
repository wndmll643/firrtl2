...
// v5: uses ALL non-clk/reset data input ports (no maxInputPorts cap).
// Larger hash sizes (inputHashSize=8, coreHashSize=10) to accommodate the
// richer input space without added address collisions.
case class HierCovParamsV5(
  inputHashSize: Int = 8,
  coreHashSize: Int = 10,
...
object HierCovUtilV5 {
  ...
  // v5: collect bits from ALL non-clk/reset UInt/SInt input ports.
  // No per-port-count cap; still limit bits extracted per port via maxBitsPerPort.
  def selectInputBits(ports: Seq[Port], params: HierCovParamsV5): Seq[(Expression, String)] = {
    ...
    // Use every port; limit bits extracted from each wide port.
  ...
  def selectRegBits(regs: Seq[DefRegister], params: HierCovParamsV5): Seq[(Expression, String)] = {
...
class HierModuleInfoV5(val mName: String,
                       val insts: scala.collection.Set[WDefInstance],
                       val regs: scala.collection.Set[DefRegister])

object HierModuleInfoV5 {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV5 = {
    val insts = gLedger.getInstances
    val regs = gLedger.findRegs
    new HierModuleInfoV5(mod.name, insts, regs)
  }
}

class InstrHierCovV5(mod: DefModule,
                     mInfo: HierModuleInfoV5,
                     extModules: Set[String],
                     params: HierCovParamsV5) {
  import HierCovUtilV5._
...
class InstrHierAssertV5(mod: DefModule, mInfo: HierModuleInfoV5) {
  private val mName = mod.name
...
class InstrHierResetV5(mod: DefModule,
                       mInfo: HierModuleInfoV5,
                       moduleInfos: scala.collection.Map[String, HierModuleInfoV5]) {
  private val mName = mod.name
  private val insts = mInfo.insts
  ...
  def addMetaReset(metaReset: Port)(s: Statement): Statement = s match {
    case Connect(info, loc, expr) if regs.exists(r => r.name == loc.serialize) =>
      ...
      reg.tpe match {
        ...
        case SIntType(IntWidth(w)) => Connect(info, loc, Mux(WRef(metaReset), UIntLiteral(0, IntWidth(w)), expr))
        case _ => s
      }
...
class hierCoverage_v5 extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV5]()
  private val params      = HierCovParamsV5()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV5(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      new InstrHierCovV5(m, moduleInfos(m.name), extModules, params).instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      new InstrHierAssertV5(m, moduleInfos(m.name)).instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      new InstrHierResetV5(m, moduleInfos(m.name), moduleInfos).instrument()
    }
