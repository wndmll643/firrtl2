// v6b: redesigned hierCoverage — control registers only (core hash) + CONTROL input ports
// (ports that feed mux selects) for input hash. Fully focused on control-flow state:
// both internal (ctrl regs) and external (ctrl inputs from parent). Eliminates all data noise.
//
// Changes vs v6a:
//   - selectControlInputBits: uses ONLY ports in ctrlSrcs("Port") with Input direction
//   - HierCovParamsV6b: inputHashSize reduced to 4 (control inputs are few/narrow)
package hier_cov
...
object HierCovUtilV6b {
  ...
  def selectControlInputBits(ports: Seq[Port], ctrlPortNames: Set[String], params: HierCovParamsV6b): Seq[(Expression, String)] = {
...
object HierModuleInfoV6b {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV6b = {
    ...
    new HierModuleInfoV6b(mod.name, insts, regs, ctrlRegs, dirInRegs, ctrlPortNames)
  }
}

class InstrHierCovV6b(mod: DefModule,
                      mInfo: HierModuleInfoV6b,
                      extModules: Set[String],
                      params: HierCovParamsV6b) {
  import HierCovUtilV6b._
...
class InstrHierAssertV6b(mod: DefModule, mInfo: HierModuleInfoV6b) {
  private val mName = mod.name
...
class InstrHierResetV6b(mod: DefModule, mInfo: HierModuleInfoV6b, moduleInfos: scala.collection.Map[String, HierModuleInfoV6b]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v6b extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV6b]()
  private val params = HierCovParamsV6b()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV6b(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV6b(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV6b(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV6b(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV6b.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV6b.selectControlInputBits(m.ports, mInfo.ctrlPortNames, params)
            val regBits = HierCovUtilV6b.selectControlRegBits(mInfo.ctrlRegs, mInfo.dirInRegs, params)
