// v4_3746_fix: same as v4_3746 (hier coverage + writeCoverageSummary) but fixes
// the SInt metaReset bug — uses SIntLiteral(0) instead of UIntLiteral(0) when
// resetting SInt-typed registers, avoiding FIRRTL type-mismatch errors on designs
// with SInt regs (e.g. DivSqrtRawFN.sExp_Z in micro_rocket_574 FPU).
package hier_cov
...
case class HierCovParamsV4_3746_Fix(
  inputHashSize: Int = 6,
  coreHashSize: Int = 8,
...
object HierCovUtilV4_3746_Fix {
  ...
  def selectInputBits(ports: Seq[Port], params: HierCovParamsV4_3746_Fix): Seq[(Expression, String)] = {
  ...
  def selectRegBits(regs: Seq[DefRegister], params: HierCovParamsV4_3746_Fix): Seq[(Expression, String)] = {
...
class HierModuleInfoV4_3746_Fix(val mName: String,
                            val insts: scala.collection.Set[WDefInstance],
                            val regs: scala.collection.Set[DefRegister])

object HierModuleInfoV4_3746_Fix {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV4_3746_Fix = {
    val insts = gLedger.getInstances
    val regs = gLedger.findRegs
    new HierModuleInfoV4_3746_Fix(mod.name, insts, regs)
  }
}

class InstrHierCovV4_3746_Fix(mod: DefModule,
                          mInfo: HierModuleInfoV4_3746_Fix,
                          extModules: Set[String],
                          params: HierCovParamsV4_3746_Fix) {
  import HierCovUtilV4_3746_Fix._
...
class InstrHierAssertV4_3746_Fix(mod: DefModule, mInfo: HierModuleInfoV4_3746_Fix) {
  private val mName = mod.name
...
class InstrHierResetV4_3746_Fix(mod: DefModule, mInfo: HierModuleInfoV4_3746_Fix, moduleInfos: scala.collection.Map[String, HierModuleInfoV4_3746_Fix]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v4_3746_fix extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV4_3746_Fix]()
  private val params = HierCovParamsV4_3746_Fix()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV4_3746_Fix(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV4_3746_Fix(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV4_3746_Fix(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV4_3746_Fix(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], topName: String): Unit = {
    ...
    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV4_3746_Fix.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV4_3746_Fix.selectInputBits(m.ports, params)
            val regBits = HierCovUtilV4_3746_Fix.selectRegBits(mInfo.regs.toSeq, params)
            val submodInsts = mInfo.insts.count(inst => !extModules.contains(inst.module))
            val inputHashEff = if (inputBits.nonEmpty) Math.min(params.inputHashSize, inputBits.size) else 0
            val coreBitCount = regBits.size + submodInsts * params.submodHashSize
            val coreHashEff = if (coreBitCount > 0) Math.min(params.coreHashSize, coreBitCount) else 0
    ...
    val perModule = moduleInfos.keys.toSeq.sorted.map { mName =>
      ...
      s"  ${mName}: covMapSize=${covSize}, instances=${instCnt}\n"
    }
    ...
    val text =
      ...
      s"Total coverage points (hier_cov): ${totalCov}\n" +
