...
object HierCovUtilV4Fix {
  ...
  def selectInputBits(ports: Seq[Port], params: HierCovParamsV4Fix): Seq[(Expression, String)] = {
  ...
  def selectRegBits(regs: Seq[DefRegister], params: HierCovParamsV4Fix): Seq[(Expression, String)] = {
...
class HierModuleInfoV4Fix(val mName: String,
                          val insts: scala.collection.Set[WDefInstance],
                          val regs: scala.collection.Set[DefRegister])

object HierModuleInfoV4Fix {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV4Fix = {
    val insts = gLedger.getInstances
    val regs = gLedger.findRegs
    new HierModuleInfoV4Fix(mod.name, insts, regs)
  }
}

class InstrHierCovV4Fix(mod: DefModule,
                        mInfo: HierModuleInfoV4Fix,
                        extModules: Set[String],
                        params: HierCovParamsV4Fix) {
  import HierCovUtilV4Fix._
...
class InstrHierAssertV4Fix(mod: DefModule, mInfo: HierModuleInfoV4Fix) {
  private val mName = mod.name
...
class InstrHierResetV4Fix(mod: DefModule, mInfo: HierModuleInfoV4Fix, moduleInfos: scala.collection.Map[String, HierModuleInfoV4Fix]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v4_fix extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV4Fix]()
  private val params = HierCovParamsV4Fix()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV4Fix(m, gLedger)
    }

    val extModules = circuit.modules.filter(_.isInstanceOf[ExtModule]).map(_.name).toSet

    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV4Fix(m, moduleInfos(m.name), extModules, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV4Fix(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV4Fix(m, moduleInfos(m.name), moduleInfos)
