// v7b: v7 with v6b's control-input-only selection for input hash.
// Based on v7 with one change: input hash uses ONLY control input ports
// (ports that feed mux selects) instead of data input ports.
// All other v7 features preserved: dynamic log2 hash sizing, extmodule I/O proxy,
// raised caps (maxInputHashSize=10, maxCoreHashSize=12), minHashSize floor,
// raised maxRegBits=256.
package hier_cov
...
object HierCovUtilV7b {
  ...
  def selectControlInputBits(ports: Seq[Port], ctrlPortNames: Set[String], params: HierCovParamsV7b): Seq[(Expression, String)] = {
...
object HierModuleInfoV7b {
  def apply(mod: DefModule, gLedger: graphLedger): HierModuleInfoV7b = {
    ...
    new HierModuleInfoV7b(mod.name, insts, regs, ctrlRegs, dirInRegs, ctrlPortNames)
  }
}

class InstrHierCovV7b(mod: DefModule,
                      mInfo: HierModuleInfoV7b,
                      extModules: Set[String],
                      extModulePorts: scala.collection.Map[String, Seq[Port]],
                      params: HierCovParamsV7b) {
  import HierCovUtilV7b._
...
class InstrHierAssertV7b(mod: DefModule, mInfo: HierModuleInfoV7b) {
  private val mName = mod.name
...
class InstrHierResetV7b(mod: DefModule, mInfo: HierModuleInfoV7b, moduleInfos: scala.collection.Map[String, HierModuleInfoV7b]) {
  private val mName = mod.name
  private val insts = mInfo.insts
...
class hierCoverage_v7b extends Transform {
  ...
  private val moduleInfos = mutable.Map[String, HierModuleInfoV7b]()
  private val params = HierCovParamsV7b()
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (m <- circuit.modules) {
      ...
      gLedger.parseModule
      moduleInfos(m.name) = HierModuleInfoV7b(m, gLedger)
    }
    ...
    val instrCircuit = circuit map { m: DefModule =>
      val instrCov = new InstrHierCovV7b(m, moduleInfos(m.name), extModules, extModulePorts, params)
      instrCov.instrument()
    }

    val assertCircuit = instrCircuit map { m: DefModule =>
      val instrAssert = new InstrHierAssertV7b(m, moduleInfos(m.name))
      instrAssert.instrument()
    }

    val metaResetCircuit = assertCircuit map { m: DefModule =>
      val instrReset = new InstrHierResetV7b(m, moduleInfos(m.name), moduleInfos)
  ...
  private def writeCoverageSummary(circuit: Circuit, extModules: Set[String], extModulePorts: scala.collection.Map[String, Seq[Port]], topName: String): Unit = {
    ...
    def dynamicHash(n: Int, maxH: Int): Int = {
      ...
      else Math.max(params.minHashSize, Math.min(maxH, HierCovUtilV7b.log2Ceil(n + 1)))
    }

    def covMapSizeOf(moduleName: String): Long = {
      moduleMap.get(moduleName) match {
        case Some(m: Module) =>
          val (_, hasClk) = HierCovUtilV7b.hasClock(m)
          if (!hasClk) {
            0L
          } else {
            val mInfo = moduleInfos(moduleName)
            val inputBits = HierCovUtilV7b.selectControlInputBits(m.ports, mInfo.ctrlPortNames, params)
            val regBits = HierCovUtilV7b.selectControlRegBits(mInfo.ctrlRegs, mInfo.dirInRegs, params)
    ...
    val text =
      ...
      s"Total coverage points (hier_cov_v7b control-input): ${totalCov}\n" +
